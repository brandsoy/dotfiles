...
map(
	'n',
	'<leader>mL',
	'i[]()<Esc>F(a<C-r>+<Esc>F[a',
	'Insert buffered markdown link'
)
...
map('n', '<leader>mp', function()
	...
	if not platform then
		vim.notify(
			'Unsupported platform (no display detected)',
	...
	elseif platform == 'x11' then
		...
		local check =
			vim.fn.system('xclip -selection clipboard -t TARGETS -o 2>/dev/null')
		...
		write_cmd =
			string.format('xclip -selection clipboard -t image/png -o > %q', outpath)
	elseif platform == 'wayland' then
		-- Check for wl-paste
		if vim.fn.executable('wl-paste') ~= 1 then
			vim.notify(
				'wl-paste not found. Install with: sudo apt install wl-clipboard',
				vim.log.levels.ERROR
			)
			return
		end
