...
function M.setup()
	...
	local function grep_project_hidden()
		fzf.live_grep({
			cwd = project_root_or_cwd(),
			rg_opts = "--column --line-number --no-heading --color=always --smart-case --max-columns=4096 --hidden --glob '!.git' --glob '!node_modules' --glob '!.venv' --glob '!dist' --glob '!build' -e",
		})
	end
	...
	map('n', '<leader>fG', grep_project_hidden, { desc = 'Live grep (including hidden files)' })
