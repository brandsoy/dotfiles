...
require_theme() {
    if ! theme_exists "$1"; then
        ...
        printf "Run 'theme-sync list' to view available themes.\n" >&2
        exit 1
    fi
}
...
detect_system_mode() {
    ...
    Darwin)
        ...
        ;;
    *) die "Unsupported OS for automatic mode detection: $(uname -s)" ;;
    esac
}
...
apply_system_mode_theme() {
    ...
    [[ -n "$theme" ]] || die "No theme configured for $mode mode. Use: theme-sync mode-set $mode <theme>"
...
apply_cli_themes() {
    # Bat, btop, and Neovim theme state.
...
main() {
    ...
    apply)
        local theme
        theme="$(current_theme)"
        [[ -n "$theme" ]] || die "No current theme set. Use: theme-sync set <theme>"
        apply_theme "$theme"
        ;;
    mode-set)
        set_mode_theme "${2:-}" "${3:-}"
